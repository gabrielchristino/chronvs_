...
static lv_obj_t *label(const char *caption, int y, int width, const lv_font_t *font) {
    lv_obj_t *obj = chronvs_Relogio_label(page, caption, y, font);
...
static void update_title(void) {
    ...
    char count[24]; snprintf(count, sizeof(count), "%u / %u", Notas_text_length(editor.text), CHRONVS_REMINDER_CHARS);
    error(count);
...
static void action(lv_event_t *event) {
    ...
    if (id == -1) {
        chronvs_time_t now;
        if (!chronvs_Relogio_time(&now)) { error("Horário indisponível"); return; }
        draft.hour = now.hour; draft.minute = (now.minute + 1) % 60;
        if (!draft.minute) draft.hour = (now.hour + 1) % 24;
        draft.title[0] = 0; draft.done = 0;
        Notas_editor_load(&editor, ""); symbols = false; view = TITLE;
    } else if (id >= 0) { selected = id; view = ITEM; }
    else if (id == -2) {
        chronvs_time_t now;
        if (!chronvs_Relogio_time(&now)) { error("Horario indisponivel"); return; }
...
static void key_event(lv_event_t *event) {
    ...
    if (key == 30) Notas_editor_shift(&editor);
    else if (key == 31) Notas_editor_backspace(&editor);
    else if (key == 34) { Notas_editor_confirm(&editor); symbols = !symbols; }
    else {
        unsigned group = symbols && key < 13 ? key + Notas_SYMBOL_FIRST : key;
        if (Notas_text_length(editor.text) >= CHRONVS_REMINDER_CHARS &&
            (key == 32 || editor.pending_key != (int)group || lv_tick_elaps(editor.last_tap) >= Notas_TAP_MS)) {
            error("Limite de 40 caracteres"); return;
        }
        if (key == 32) Notas_editor_insert(&editor, " ");
        else Notas_editor_key(&editor, group, lv_tick_get());
    }
    update_title();
}
...
static void text_event(lv_event_t *event) {
    ...
    if (code == LV_EVENT_RELEASED && !dragged) {
        ...
        Notas_editor_move(&editor, lv_label_get_letter_on(caption, &relative));
        update_title();
    }
}
static void title_page(void) {
    label("Título do lembrete", 48, 230, &chronvs_Notas_font);
    status = label("", 76, 230, &chronvs_Notas_font);
    ...
    lv_obj_set_style_text_font(text, &chronvs_Notas_font, 0);
...
static void rebuild(void) {
    ...
    page = lv_obj_create(root); chronvs_Relogio_surface(page);
    ...
    if (view == TITLE) title_page();
        ...
        if (view == LIST) {
            ...
            for (unsigned j=0; j<count; ++j) {
                ...
                lv_obj_t *row=chronvs_Relogio_button(list, caption, 0, j*64, 276, 54, action, order[j]);
                lv_obj_t *name=lv_obj_get_child(row,0); lv_obj_set_width(name,244);
                lv_obj_set_style_text_font(name,&chronvs_Notas_font,0);
                lv_label_set_long_mode(name,LV_LABEL_LONG_DOT); lv_obj_center(name);
            }
            if (!count) label("Nenhum lembrete", 180, 260, &chronvs_Notas_font);
            lv_obj_t *add=chronvs_Relogio_action(page,"Criar",0,314,CHRONVS_UI_ACTION_WIDTH,false,action,-1);
            ...
            label(view==HOUR ? "Escolha a hora" : "Escolha o minuto", 92, 240, &chronvs_Notas_font);
            ...
            lv_obj_t *caption=label(r->title,116,280,&chronvs_Notas_font);
            ...
            if (view==ITEM) label(r->done?"Concluído":"Agendado",280,220,&chronvs_Notas_font);
            chronvs_Relogio_action(page,view==REVIEW?"Salvar":"Excluir",0,314,CHRONVS_UI_ACTION_WIDTH,
                view==ITEM,action,view==REVIEW?-2:-3);
        } else {
            label(view==DELETE?"Excluir este lembrete?":"Descartar rascunho?",148,280,&chronvs_Notas_font);
            chronvs_Relogio_action(page,"Cancelar",-66,242,CHRONVS_UI_PAIR_WIDTH,true,action,-5);
            chronvs_Relogio_action(page,view==DELETE?"Excluir":"Descartar",66,242,CHRONVS_UI_PAIR_WIDTH,false,
...
void chronvs_Calendario_reminders_open(lv_obj_t *parent,int year,int month,int day) {
    ...
    root=lv_obj_create(parent); chronvs_Relogio_surface(root);
    view=LIST; rebuild();
}
bool chronvs_Calendario_reminders_active(void) { return root!=NULL; }
void chronvs_Calendario_reminders_poll(void) {
    ...
    if (view==TITLE && Notas_editor_expire(&editor,lv_tick_get())) update_title();
    if (arc && value) {
