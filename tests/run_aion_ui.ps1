...
try {
    ...
    $arguments = @('-std=c11', '-O0', '-DLV_CONF_INCLUDE_SIMPLE', '-I', 'src', '-I', 'tests/aion_stubs', '-I', $lvglRoot,
        'tests/aion_ui_test.c', 'src/services/aion_service.c', 'src/core/calendar.c', 'src/core/mnemo_text.c', 'src/ui/mnemo_font.c', 'src/ui/aion_widgets.c', 'src/ui/control_style.c') + $sources + @('-o', '.pio/host-tests/aion_ui_test.exe')
    ...
    if ($System) {
        ...
        $arguments += @('src/apps/watch_app.c','src/apps/app_list_app.c','src/apps/app_catalog.c',
            ...
            'src/apps/mnemo_app.c','src/services/mnemo_service.c','src/ui/app_input.c')
        $arguments += @('-lm')
        ...
        $arguments += @('src/apps/hemera_app.c','src/apps/hemera_reminders.c','src/ui/aion_alert.c')
