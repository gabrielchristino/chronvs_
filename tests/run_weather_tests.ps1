...
try {
    ...
    $arguments = @('-std=c11','-O0','-Wall','-Wextra','-Werror','-I','tests/weather_stubs',
        '-I','tests/Relogio_stubs','-I','src','-I',$jsonRoot,'tests/weather_test.c',
    ...
    & gcc -std=c11 -Wall -Wextra -Werror -I tests/weather_stubs -I tests/Relogio_stubs -I src tests/wifi_session_test.c -o .pio/host-tests/wifi_session_test.exe
    ...
    if ($UI) {
        ...
        $arguments = @('-std=c11','-O0','-DLV_CONF_INCLUDE_SIMPLE','-I','src','-I','tests/Relogio_stubs',
            ...
            'src/ui/Relogio_widgets.c','src/ui/control_style.c','src/ui/app_input.c','src/ui/Notas_font.c','src/ui/weather_icon.c','src/ui/weather_art.c','src/ui/weather_face.c',
